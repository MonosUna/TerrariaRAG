...
class TerrariaRAG:
    ...
    [
        ...
        {
            "GeneralAgent": "Для создания грани ночи требуется следующие материалы: меч Бич света или Кровавый Забиватель, Мурамаса, Травяной клинок и меч Вулкан. Крафт происходит на демоническом или багровом алтаре.",
        },
        {
    ...
    Привет! Сейчас расскажу. Грань ночи - это мощное оружие ближнего боя, которое особенно эффективно в середине игры Terraria.
    ...
    SYSTEM_PROMPT__REDIRECT_TO_AGENTS_OLD = """
    ...
    Твоя задача - определить, каких агентов нужно привлечь для ответа на вопрос
    и переформулировать вопросы для каждого агента.

    Инструкция:
    1. Проанализируй вопрос пользователя и раздели на отдельные вопросы для других агентов.
    2. Для каждого вопроса, переформулируй его так, чтобы агенты отвечали на один простой вопрос.
    ...
    4. В ответе не упоминай ничего о других агентах
